...
fn preferred_work_sizes(n: usize, m: usize, group_size_for_1d: usize, group_size_for_2d: usize, item_row_count: usize, item_col_count: usize, are_swapped_dims: bool, is_mul: bool) -> (usize, usize, usize, usize)
{
    if m <= item_col_count && !is_mul {
        let n2 = (((n + item_row_count - 1) / item_row_count + group_size_for_1d - 1) / group_size_for_1d) * group_size_for_1d;
        if !are_swapped_dims {
            (group_size_for_1d, 1, n2, 1)
        } else {
            (1, group_size_for_1d, 1, n2)
        }
    } else if n <= item_row_count && !is_mul {
        let m2 = (((m + item_col_count - 1) / item_col_count + group_size_for_1d - 1) / group_size_for_1d) * group_size_for_1d;
        if !are_swapped_dims {
        ...
        if !are_swapped_dims {
            ...
            ((group_size_for_2d + 1) / 2, (group_size_for_2d + 1) / 2, m2, n2)
        }
    } else {
        let n2 = (((n + item_row_count - 1) / item_row_count + group_size_for_2d - 1) / group_size_for_2d) * group_size_for_2d;
        let m2 = (((m + item_col_count - 1) / item_col_count + group_size_for_2d - 1) / group_size_for_2d) * group_size_for_2d;
        if !are_swapped_dims {
            (group_size_for_2d, group_size_for_2d, n2, m2)
        } else {
            (group_size_for_2d, group_size_for_2d, m2, n2)
        }
    }
}
...
impl ClBackend
{
    ...
    fn check_and_enqueue_nd_range_for_fun(&self, kernel_name: &str, a: &BackendArray, b: &BackendArray, n: usize, m: usize, item_row_count: usize, item_col_count: usize, are_swapped_dims: bool) -> Result<()>
    {
        self.check_and_enqueue_nd_range2(kernel_name, a, b, |a2, b2| {
                ...
                let n2 = n as u64;
                let m2 = m as u64;
                let (n3, m3, n4, m4) = preferred_work_sizes(n, m, inner.group_size_for_1d, inner.group_size_for_2d, item_row_count, item_col_count, are_swapped_dims, false);
                unsafe {
    ...
    fn check_and_enqueue_nd_range_for_op(&self, kernel_name: &str, a: &BackendArray, b: &BackendArray, c: &BackendArray, n: usize, m: usize, item_row_count: usize, item_col_count: usize, are_swapped_dims: bool) -> Result<()>
    {
    ...
    fn check_and_enqueue_nd_range_for_scalar(&self, kernel_name: &str, a: &BackendArray, b: f32, c: &BackendArray, n: usize, m: usize, item_row_count: usize, item_col_count: usize, are_swapped_dims: bool) -> Result<()>
    {
    ...
    fn check_and_enqueue_nd_range_for_fun_and_tiles(&self, kernel_name: &str, a: &BackendArray, b: &BackendArray, n: usize, m: usize, item_row_count: usize, item_col_count: usize, are_swapped_dims: bool) -> Result<()>
    {
        self.check_and_enqueue_nd_range2(kernel_name, a, b, |a2, b2| {
                if a2.len != n * m {
                    return Err(Error::BackendArrayElemCount(a2.len, n * m));
                ...
                unsafe {
                    ...
                    .set_arg(&a_mem)
                    .set_arg(&b_mem)
                    .set_arg_local_buffer(n3 * item_row_count * m3 * item_col_count * size_of::<f32>())
                    .set_arg(&n2)
                    .set_arg(&m2)
                    .set_local_work_sizes(&[n3, m3])
                    .set_global_work_sizes(&[n4, m4])
    ...
    fn check_and_enqueue_nd_range_for_repeat_col(&self, kernel_name: &str, a: &BackendArray, b: &BackendArray, n: usize, m: usize, item_row_count: usize, item_col_count: usize, are_swapped_dims: bool) -> Result<()>
    {
        self.check_and_enqueue_nd_range2(kernel_name, a, b, |a2, b2| {
                if a2.len != n {
    ...
    fn check_and_enqueue_nd_range_for_repeat_row(&self, kernel_name: &str, a: &BackendArray, b: &BackendArray, n: usize, m: usize, item_row_count: usize, item_col_count: usize, are_swapped_dims: bool) -> Result<()>
    {
        self.check_and_enqueue_nd_range2(kernel_name, a, b, |a2, b2| {
                if a2.len != m {
